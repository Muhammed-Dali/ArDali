...
function compileEffect(effect, index) {
  ...
  if (effect.effect === 'preamp') {
    ...
    return {
      create: [
        `const ${nodeName} = audioContext.createGain();`,
        `${nodeName}.gain.value = ${gain.toFixed(8)};`
      ].join('\n')
    };
  }
  ...
  if (effect.effect === 'low_shelf') {
    ...
    return {
      create: [
        `const ${nodeName} = audioContext.createBiquadFilter();`,
        `${nodeName}.type = 'lowshelf';`,
        `${nodeName}.frequency.value = ${freq};`,
        `${nodeName}.gain.value = ${gain};`,
        `${nodeName}.Q.value = ${q};`
      ].join('\n')
    };
  }
  ...
  if (effect.effect === 'compressor') {
    ...
    return {
      create: [
        `const ${nodeName}_comp = audioContext.createDynamicsCompressor();`,
        `${nodeName}_comp.threshold.value = ${threshold};`,
        `${nodeName}_comp.ratio.value = ${ratio};`,
        `${nodeName}_comp.attack.value = ${(attackMs / 1000).toFixed(6)};`,
        `${nodeName}_comp.release.value = ${(releaseMs / 1000).toFixed(6)};`,
        `${nodeName}_comp.knee.value = ${knee};`,
        `const ${nodeName}_makeup = audioContext.createGain();`,
        `${nodeName}_makeup.gain.value = ${makeupGain.toFixed(8)};`,
  ...
  if (effect.effect === 'limiter') {
    ...
    return {
      create: [
        `const ${nodeName} = audioContext.createDynamicsCompressor();`,
        `${nodeName}.threshold.value = ${ceiling};`,
        `${nodeName}.ratio.value = 20;`,
        `${nodeName}.attack.value = ${(attackMs / 1000).toFixed(6)};`,
        `${nodeName}.release.value = ${(releaseMs / 1000).toFixed(6)};`,
        `${nodeName}.knee.value = 0;`
      ].join('\n')
    };
  }
...
function compilePresetWebAudio(preset) {
  ...
  lines.push('      chain: [');
  preset.chain.forEach((effect, idx) => {
    lines.push(`        { effect: ${JSON.stringify(effect.effect)}, node: n${idx}, input: n${idx}_in, output: n${idx}_out }${idx === preset.chain.length - 1 ? '' : ','}`);
  });
  lines.push('      ]');
  lines.push('    },');
  lines.push('    security: {');
