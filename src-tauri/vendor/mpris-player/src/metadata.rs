...
impl Metadata {
    ...
    pub fn to_hashmap(&self) -> HashMap<String, Variant<Box<dyn RefArg + 'static>>> {
        ...
        if self.track_id.is_some() {
            if let Ok(path) = Path::new(self.track_id.clone().unwrap()) {
                let x = Box::new(path) as Box<dyn RefArg>;
                metadata.insert("mpris:trackid".to_string(), Variant(x));
            }
        }

        if self.length.is_some() {
            let x = Box::new(self.length.unwrap()) as Box<dyn RefArg>;
