...
pub struct MprisPlayer {
    ...
    volume: Cell<f64>,                     // R/W
    position: Cell<i64>,                   // R
    position_updated_at: Cell<Option<Instant>>,
    ...
    position_cb: RefCell<Vec<Rc<RefCell<dyn FnMut(i64)>>>>,
...
impl MprisPlayer {
    pub fn new(mpris_name: String, identify: String, desktop_entry: String) -> Arc<Self> {
        ...
        let mpris_player = Arc::new(MprisPlayer {
            ...
            position: Cell::new(0),
            position_updated_at: Cell::new(None),
            minimum_rate: Cell::new(1_f64),
            maximum_rate: Cell::new(1_f64),
            ...
            position_cb: RefCell::new(Vec::new()),
    ...
    pub fn process_events(&self) {
        self.connection.incoming(40u32).next();
    }
    ...
    pub fn set_playback_status(&self, value: PlaybackStatus) {
        if self.playback_status.get() != value {
            if self.playback_status.get() == PlaybackStatus::Playing {
                self.position.set(self.current_position());
            }
            self.playback_status.set(value);
            self.position_updated_at.set(if value == PlaybackStatus::Playing {
                Some(Instant::now())
            } else {
                None
            });
    ...
    pub fn set_position(&self, value: i64) {
        ...
        self.position_updated_at.set(if self.playback_status.get() == PlaybackStatus::Playing {
            Some(Instant::now())
        } else {
            None
        });
    ...
    pub fn emit_seeked(&self, position: i64) {
        let signal = OrgMprisMediaPlayer2PlayerSeeked { position };
        self.connection
            .send(signal.to_emit_message(&Path::new("/org/mpris/MediaPlayer2").unwrap()))
            .unwrap();
    }
    ...
    fn current_position(&self) -> i64 {
        let base = self.position.get();
        if self.playback_status.get() != PlaybackStatus::Playing {
            return base;
        }

        let Some(updated_at) = self.position_updated_at.get() else {
            return base;
        };

        let elapsed = updated_at.elapsed().as_micros();
        let elapsed = if elapsed > i64::MAX as u128 {
            i64::MAX
        } else {
            elapsed as i64
        };
    ...
    pub fn connect_position<F: FnMut(i64) + 'static>(&self, callback: F) {
        let cell = Rc::new(RefCell::new(callback));
        self.position_cb.borrow_mut().push(cell);
    }
...
impl OrgMprisMediaPlayer2Player for MprisPlayer {
    ...
    fn set_position(&self, _track_id: dbus::Path, position: i64) -> Result<(), Self::Err> {
        ...
        for callback in self.position_cb.borrow_mut().iter() {
            let mut closure = callback.borrow_mut();
            (&mut *closure)(position);
        }
        Ok(())
    }
